import { HttpException, HttpStatus, Inject, Injectable } from '@nestjs/common';
import { JwtService } from '@nestjs/jwt';
import { hash, compare } from 'bcrypt';
import { Users } from 'src/entities';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { CheckProofPayloadDto, UserLoginDto } from 'src/request-dto';
import { EnvironmentConfigService } from 'src/infrastructure/environment-config';
import * as crypto from 'crypto';
import {Address,Cell} from '@ton/core';
import {TonClient} from '@ton/ton';
import BN from 'bn.js';
// import nacl from 'tweetnacl';
import * as nacl from 'tweetnacl-ts'

@Injectable()
export class AuthService {
  constructor(
    @InjectRepository(Users)
    private readonly usersRepository: Repository<Users>,
    private jwtService: JwtService,
    private readonly envConfig: EnvironmentConfigService,
  ) {}

  async login(data:UserLoginDto){
    try{
      const checkWalletExists = await this.usersRepository.findOne({
        where:{
          wallet_address:data.walletAddress
        }
      })

      if(checkWalletExists){
        const payload = {
          'wallet_address':data.walletAddress
        }
        return this.generateJWT(payload);
      }

      if(!checkWalletExists){
        const createWalletEntry = await this.usersRepository.insert({
          wallet_address:data.walletAddress
        })
        if(createWalletEntry){
          const payload = {
            "wallet_address":data.walletAddress
          }
          return this.generateJWT(payload);
        }
      }
    }
    catch(error){
      throw new HttpException("Error while login",HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  /**
   * Generates a 48 bit verification payload for verifying ton wallet
   */
  async generateVerificationPayload(){
    const randomBits = crypto.randomBytes(8);

    const currentTime = Math.floor(Date.now() / 1000);
    const expirationTime = Buffer.alloc(8);
    expirationTime.writeBigUint64BE(BigInt(currentTime + this.envConfig.getTonPayloadttl()));
    const payload = Buffer.concat([randomBits, expirationTime]);
  
    const hmac = crypto.createHmac("sha256", this.envConfig.getTonSharedSecret());
    hmac.update(payload);
    const signature = hmac.digest();
  
    const finalPayload = Buffer.concat([payload, signature]);
  
    const payloadHex = finalPayload.subarray(0, 32).toString("hex");

    return payloadHex;
  }

  /**
   * 
   */
  async checkProof(data:CheckProofPayloadDto){
    try{
      const {proof,address,network} = data;

      const payload = Buffer.from(proof.payload,"hex");
      if (payload.length !== 32) {
        throw new HttpException(`Invalid proof payload length,got${payload.length}, expected 32`,HttpStatus.BAD_REQUEST);
      }

      const mac = crypto.createHmac("sha256",this.envConfig.getTonSharedSecret());
      mac.update(payload.subarray(0,16));
      const payloadSignatureBytes = mac.digest();

      const signatureValid = payload.subarray(16).equals(payloadSignatureBytes.subarray(0, 16));
      if(!signatureValid){
        throw new HttpException(`invalid payload signature`,HttpStatus.BAD_REQUEST)
      }

    const now = Math.floor(Date.now() / 1000);
    const expireBytes = payload.subarray(8, 16);
    const expireTime = expireBytes.readBigUint64BE();
    if (BigInt(now) > expireTime) {
      throw new HttpException('payload expired',HttpStatus.BAD_REQUEST);
    }

    if (now > proof.timestamp + this.envConfig.getTonProofttl()) {
      throw new HttpException('payload expired',HttpStatus.BAD_REQUEST);
    }

    if (proof.domain.value !== this.envConfig.getTonDomain()) {
      throw new HttpException('Invalid domain value',HttpStatus.BAD_REQUEST);
    }

    if (proof.domain.lengthBytes !== proof.domain.value.length) {
      throw new HttpException('domain length missmatch',HttpStatus.BAD_REQUEST);
    }

    const _address = address.value
    const _network = network.value;


    const parsedAddress = Address.parse(_address);

    const wc = Buffer.alloc(4);
    wc.writeInt32BE(parsedAddress.workChain);

    const ts = Buffer.alloc(8);
    ts.writeBigUint64LE(BigInt(proof.timestamp));

    const dl = Buffer.alloc(4);
    dl.writeUint32LE(proof.domain.value.length);

    const tonProofPrefix = "ton-proof-item-v2/";
    const msg = Buffer.concat([
      Buffer.from(tonProofPrefix),
      wc,
      parsedAddress.hash,
      dl,
      Buffer.from(proof.domain.value),
      ts,
      Buffer.from(proof.payload),
    ]);

    const msgHash = crypto.createHash("sha256").update(msg).digest();

    const tonConnectPrefix = "ton-connect";
    const fullMsg = Buffer.concat([
      Buffer.from([0xff, 0xff]),
      Buffer.from(tonConnectPrefix),
      msgHash,
    ]);

    const fullMsgHash = crypto.createHash("sha256").update(fullMsg).digest();

    const TON_ENV = this.envConfig.getTonEnvironment();

    const client = new TonClient({
      endpoint: `https://${
        _network === "TESTNET" ? "testnet." : ""
      }toncenter.com/api/v2/jsonRPC`,
    });

    const executionRes = await client.runMethodWithError(
      parsedAddress,
      "get_public_key"
    );
    let pubkey: Buffer;

    if (executionRes.exit_code === 0) {
      const pubkeyNum = executionRes.stack.readBigNumber();
      const pubkeyBn = new BN(pubkeyNum.toString());
      pubkey = pubkeyBn.toBuffer("be", 32);
    } else {
      const codes = {
        V1R1: "te6cckEBAQEARAAAhP8AIN2k8mCBAgDXGCDXCx/tRNDTH9P/0VESuvKhIvkBVBBE+RDyovgAAdMfMSDXSpbTB9QC+wDe0aTIyx/L/8ntVEH98Ik=",
        V1R2: "te6cckEBAQEAUwAAov8AIN0gggFMl7qXMO1E0NcLH+Ck8mCBAgDXGCDXCx/tRNDTH9P/0VESuvKhIvkBVBBE+RDyovgAAdMfMSDXSpbTB9QC+wDe0aTIyx/L/8ntVNDieG8=",
        V1R3: "te6cckEBAQEAXwAAuv8AIN0gggFMl7ohggEznLqxnHGw7UTQ0x/XC//jBOCk8mCBAgDXGCDXCx/tRNDTH9P/0VESuvKhIvkBVBBE+RDyovgAAdMfMSDXSpbTB9QC+wDe0aTIyx/L/8ntVLW4bkI=",
        V2R1: "te6cckEBAQEAVwAAqv8AIN0gggFMl7qXMO1E0NcLH+Ck8mCDCNcYINMf0x8B+CO78mPtRNDTH9P/0VExuvKhA/kBVBBC+RDyovgAApMg10qW0wfUAvsA6NGkyMsfy//J7VShNwu2",
        V2R2: "te6cckEBAQEAYwAAwv8AIN0gggFMl7ohggEznLqxnHGw7UTQ0x/XC//jBOCk8mCDCNcYINMf0x8B+CO78mPtRNDTH9P/0VExuvKhA/kBVBBC+RDyovgAApMg10qW0wfUAvsA6NGkyMsfy//J7VQETNeh",
        V3R1: "te6cckEBAQEAYgAAwP8AIN0gggFMl7qXMO1E0NcLH+Ck8mCDCNcYINMf0x/TH/gjE7vyY+1E0NMf0x/T/9FRMrryoVFEuvKiBPkBVBBV+RDyo/gAkyDXSpbTB9QC+wDo0QGkyMsfyx/L/8ntVD++buA=",
        V3R2: "te6cckEBAQEAcQAA3v8AIN0gggFMl7ohggEznLqxn3Gw7UTQ0x/THzHXC//jBOCk8mCDCNcYINMf0x/TH/gjE7vyY+1E0NMf0x/T/9FRMrryoVFEuvKiBPkBVBBV+RDyo/gAkyDXSpbTB9QC+wDo0QGkyMsfyx/L/8ntVBC9ba0=",
        V4R1: "",
        V4R2: "",
      };

      const boc = Cell.fromBase64(proof.state_init);
      const code = boc.refs[0];
      const data = boc.refs[1];
      const version = code.toBoc().toString("base64");

      switch (version) {
        case codes.V1R1:
        case codes.V1R2:
        case codes.V1R3:
        case codes.V2R1:
        case codes.V2R2: {
          // skip seqno
          pubkey = data.asSlice().skip(32).loadBuffer(32);
          break;
        }
        case codes.V3R1:
        case codes.V3R2:
        case codes.V4R1:
        case codes.V4R2: {
          // skip seqno, walletId
          pubkey = data.asSlice().skip(64).loadBuffer(32);
          break;
        }
        default: {
          throw new HttpException('unsupported wallet version',HttpStatus.BAD_REQUEST);
        }
      }
    }

    const proofSignatureBytes = Buffer.from(proof.signature, "base64");
    const verified = nacl.sign_detached_verify(
      fullMsgHash,
      proofSignatureBytes,
      pubkey
    )

    if (!verified) {
      throw new HttpException('Verification Failed',HttpStatus.BAD_REQUEST)
    }

    const claims = {
      address: parsedAddress.toString(),
    };
    const SHARED_SECRET = this.envConfig.getTonSharedSecret();

    const token = this.jwtService.sign(claims,{
      secret:SHARED_SECRET,
      expiresIn:now+this.envConfig.getTonPayloadttl()
    })  

    return token
  }
  catch(error){
    throw error
  }
}
   


  generateJWT(payload: any) {
    return this.jwtService.sign(payload, {
      secret: this.envConfig.getJwtSecret(),
      expiresIn: this.envConfig.getJwtExpiry(),
    });
  }

  validateJWT(token: string): any {
    try {
      const jwtOptions = {
        secret: this.envConfig.getJwtSecret(),
      };
      const decode = this.jwtService.verify(token, jwtOptions);
      return decode;
    } catch (error) {
      throw new HttpException('Invalid Token', HttpStatus.UNAUTHORIZED);
    }
  }
}